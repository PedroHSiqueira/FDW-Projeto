  ...
  return (
    <View className="flex-1 bg-[#1A1A1A] justify-center items-center p-6">
      ...
      <View className="w-full max-w-[400px]">
        <TouchableOpacity onPress={handleSubmit(verificaLogin)}>
          <Text className="text-white mb-2">Seu Email:</Text>
          <Controller control={control} required {...register("email")} name="email" render={({ field: { onChange, value } }) => <TextInput className="bg-[#1E1E1E] border border-[#333] rounded-lg p-3 text-white" placeholder="Digite seu e-mail" placeholderTextColor="#999" onChangeText={onChange} value={value} keyboardType="email-address" autoCapitalize="none" />} />
          ...
          <TouchableOpacity className="bg-white p-3 rounded-lg items-center mb-6 mt-6">
            <Text className="font-bold text-base text-black" onPress={handleSubmit(verificaLogin)}>
              Entrar
            </Text>
          </TouchableOpacity>
          ...
          <View className="flex-row justify-between">
            <TouchableOpacity>
              <Link href="/registro" className="text-white font-semibold">
                Registrar-se
              </Link>
            </TouchableOpacity>
            <TouchableOpacity>
